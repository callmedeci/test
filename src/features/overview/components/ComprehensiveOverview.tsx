...
import { Heart, Dumbbell, Target, TrendingUp, Calendar, User, Calculator } from 'lucide-react';
...
export default async function ComprehensiveOverview() {
  ...
  const bmiData = getBMIStatus(profile?.height_cm ?? null, profile?.current_weight_kg ?? null);
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="font-semibold text-green-800">{profile?.biological_sex || 'N/A'}</p>
            </div>
            <div className="text-center">
          ...
          <CardContent>
            {profile?.current_weight_kg && profile?.target_weight_1month_kg ? (
              <div className="space-y-2">
                ...
                <Progress 
                  value={Math.abs((profile.current_weight_kg - profile.target_weight_1month_kg) / profile.current_weight_kg * 100)} 
                  className="h-3" 
                />
                <p className="text-xs text-green-600 text-center">
                  {Math.abs(profile.current_weight_kg - profile.target_weight_1month_kg).toFixed(1)} kg to goal
                </p>
              </div>
            ) : (
          ...
          <CardContent>
            {profile?.bf_current && profile?.bf_target ? (
              <div className="space-y-2">
                ...
                <Progress 
                  value={Math.abs((profile.bf_current - profile.bf_target) / profile.bf_current * 100)} 
                  className="h-3" 
                />
                <p className="text-xs text-green-600 text-center">
                  {Math.abs(profile.bf_current - profile.bf_target).toFixed(1)}% to goal
                </p>
              </div>
            ) : (
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-green-700">{userPlan?.target_protein_g || 0}g</span>
                </div>
                <Progress value={userPlan?.target_protein_g ? (userPlan.target_protein_g / 200) * 100 : 0} className="h-2" />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-green-700">Carbs</span>
                  <span className="text-green-700">{userPlan?.target_carbs_g || 0}g</span>
                </div>
                <Progress value={userPlan?.target_carbs_g ? (userPlan.target_carbs_g / 300) * 100 : 0} className="h-2" />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-green-700">Fat</span>
                  <span className="text-green-700">{userPlan?.target_fat_g || 0}g</span>
                </div>
                <Progress value={userPlan?.target_fat_g ? (userPlan.target_fat_g / 100) * 100 : 0} className="h-2" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <ul className="space-y-2 text-sm text-green-700">
                  ...
                  {profile.preferred_diet && (
                    <li>• Your dietary preferences: {profile.preferred_diet}</li>
                  )}
                  {profile.allergies && (
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2 text-sm text-green-700">
                  ...
                  {profile.pain_mobility_issues && profile.pain_mobility_issues.length > 0 && (
                    <li>• Consider limitations: {profile.pain_mobility_issues.join(', ')}</li>
                  )}
